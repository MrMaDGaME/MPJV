...
public:
    ...
    void AddRodCollision(std::shared_ptr<Particle> particleA, std::shared_ptr<Particle> particleB, float length);//Add a collision listener between particleA and B of type RodCollision
    void AddCableCollision(std::shared_ptr<Particle> particleA, std::shared_ptr<Particle> particleB, float length);//Add a collision listener between particleA and B of type CableCollision
    void AddInterCollision(std::shared_ptr<Particle> particleA, std::shared_ptr<Particle> particleB, float length);//Add a collision listener between particleA and B of type interpenetrationCollision
    ...
    void CheckCollision(float duration);//Iterate on all Collision's listeners to check if there is a collision. The argument is the time since last frame for debugging purpose.
